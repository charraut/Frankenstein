...
def train(args, run_name, run_dir):
    ...
    random_action = numpy_rng.uniform(low=action_low, high=action_high, size=action_size)
    ...
    for global_step in tqdm(range(args.total_timesteps)):
        if global_step < args.learning_start:
            action = numpy_rng.uniform(low=action_low, high=action_high, size=action_size)
        else:
            action, _, key = actor_output(actor_train_state.apply_fn, actor_train_state.params, obs, key)
        ...
        flag = 1.0 - terminated
        replay_buffer.push(obs, action, reward, flag)
