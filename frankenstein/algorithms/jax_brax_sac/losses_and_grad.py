...
import optax
from frankenstein.algorithms.jax_brax_sac.networks import SACNetworks, Params
from frankenstein.algorithms.jax_brax_sac.utils import Transition, PRNGKey


#############################
        ## Losses ##
#############################
def make_losses(sac_network: SACNetworks, reward_scaling: float,
                discount_factor: float, action_size: int):
  ...
  def critic_loss(q_params: Params, policy_params: Params,
    ...
    target_q = jax.lax.stop_gradient(transitions.reward * reward_scaling +
                                     transitions.discount * discount_factor *
                                     next_v)
